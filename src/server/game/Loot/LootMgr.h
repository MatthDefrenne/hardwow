...
struct TC_GAME_API LootStoreItem
{
    ...
    bool Roll(bool rate, float moreChance = 1.0f) const;                             // Checks if the entry takes it's chance (at loot generation)
...
class TC_GAME_API LootTemplate
{
    ...
    public:
        ...
        void Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId = 0, Player* player = nullptr) const;
