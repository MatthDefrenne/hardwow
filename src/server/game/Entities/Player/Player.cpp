...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
    ...
    else if (guid.IsCorpse())                          // remove insignia
    {
        ...
        if (loot->loot_type == LOOT_NONE)
        {
            ...
            bones->loot.gold = uint32(urand(50, 150) * 0.016f * std::pow(float(pLevel) / 5.76f, 2.5f));
        }
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    uint32 XP = rewarded ? 0 : uint32(quest->XPValue(this)*sWorld->getRate(RATE_XP_QUEST)  * UtilitiesManager::CalculateMoreRateXPInGroup(this, quest->GetQuestId()));
